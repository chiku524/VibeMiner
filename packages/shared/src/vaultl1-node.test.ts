...
describe('vaultl1-node', () => {
  ...
  it('pins vaultl1-bin-v0.5.3 rehost with SHA256', () => {
    const presets = buildVaultL1NodePresets();
    expect(presets.every((p) => p.nodeDownloadUrl.includes('vaultl1-bin-v0.5.3'))).toBe(true);
    expect(presets.every((p) => p.nodeBinarySha256 && p.nodeBinarySha256.length === 64)).toBe(
      true,
    );
  });

  it('uses official vaultd start flags from vaultl1 v0.5.3', () => {
    const presets = buildVaultL1NodePresets();
