...
**Tauri 2 `invoke` shapes:** Commands whose Rust handler takes a struct as a parameter named `opts` (e.g. `start_node`, `stop_node`, `start_real_mining` in `lib.rs`) require the webview to call `invoke(cmd, { opts: { … } })`, not a flat object. The bridge wraps those payloads so remote and localhost UIs stay in sync with the binary. `stop_node` used to take flat snake_case args; the remote HTTPS UI often never delivered them, so Stop looked successful while `vaultd` kept running.
