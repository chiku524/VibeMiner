...
3. **Stale Boing zip URLs in D1** — If listings still point at older `testnet-v0.1.x` tags, apply **`d1/migrations/007_boing_testnet_zip_urls_v0_1_8.sql`** (after **`006_…v0_1_7.sql`** if you are upgrading from very old rows). After deploy, the **networks API** also rewrites `testnet-v0.1.0`–`v0.1.7` URLs toward the pinned tag and normalizes legacy **`github.com/chiku524/boing.network`** release URLs to **`Boing-Network/boing.network`** in responses so the desktop app receives current zips even before you run the SQL.
...
**Current GitHub release tag:** `testnet-v0.1.8` (see Boing [TESTNET-NODE-RELEASE-CHECKLIST.md](https://github.com/Boing-Network/boing.network/blob/main/docs/TESTNET-NODE-RELEASE-CHECKLIST.md) for tag → SHA pin order)  
...
|----------|------------------------------|----------------------------------|-------------------|------------------|
| **Linux x86_64** | `Boing Linux` | `boing-linux-devnet` | `https://github.com/Boing-Network/boing.network/releases/download/testnet-v0.1.8/release-linux-x86_64.zip` | `boing-node-linux-x86_64 --data-dir {dataDir} --p2p-listen /ip4/0.0.0.0/tcp/4001 --bootnodes /ip4/73.84.106.121/tcp/4001,/ip4/73.84.106.121/tcp/4001 --rpc-port 8545 --faucet-enable` |
| **macOS Apple Silicon** | `Boing macOS aarch64` | `boing-macos-aarch64-devnet` | `https://github.com/Boing-Network/boing.network/releases/download/testnet-v0.1.8/release-macos-aarch64.zip` | `boing-node-macos-aarch64 --data-dir {dataDir} --p2p-listen /ip4/0.0.0.0/tcp/4001 --bootnodes /ip4/73.84.106.121/tcp/4001,/ip4/73.84.106.121/tcp/4001 --rpc-port 8545 --faucet-enable` |

| Field | Linux / macOS zip SHA256 (optional) |
|-------|-------------------------------------|
| **Binary SHA256** | *From published `testnet-v0.1.8` zips; run `website/scripts/network-listings-release-sql.mjs testnet-v0.1.8` in the Boing repo after CI uploads assets, then paste per-zip SHA256 here.* |
