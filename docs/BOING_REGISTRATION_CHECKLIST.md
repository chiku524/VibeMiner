...
3. **Stale Boing zip URLs in D1** — If listings still point at older `testnet-v0.1.x` tags, apply **`d1/migrations/004_boing_testnet_zip_urls_v0_1_5.sql`**. After deploy, the **networks API** also rewrites `testnet-v0.1.0`–`v0.1.4` URLs to `testnet-v0.1.5` in responses so the desktop app receives current zips even before you run the SQL.
...
|----------|------------------------------|----------------------------------|-------------------|------------------|
| **Linux x86_64** | `Boing Linux` | `boing-linux-devnet` | `https://github.com/chiku524/boing.network/releases/download/testnet-v0.1.5/release-linux-x86_64.zip` | `boing-node-linux-x86_64 --data-dir {dataDir} --p2p-listen /ip4/0.0.0.0/tcp/4001 --bootnodes /ip4/73.84.106.121/tcp/4001,/ip4/73.84.106.121/tcp/4001 --rpc-port 8545 --faucet-enable` |
| **macOS Apple Silicon** | `Boing macOS aarch64` | `boing-macos-aarch64-devnet` | `https://github.com/chiku524/boing.network/releases/download/testnet-v0.1.5/release-macos-aarch64.zip` | `boing-node-macos-aarch64 --data-dir {dataDir} --p2p-listen /ip4/0.0.0.0/tcp/4001 --bootnodes /ip4/73.84.106.121/tcp/4001,/ip4/73.84.106.121/tcp/4001 --rpc-port 8545 --faucet-enable` |

| Field | Linux / macOS zip SHA256 (optional) |
|-------|-------------------------------------|
| **Binary SHA256** | *From `testnet-v0.1.5` zips (Windows `9d5f9abf…`, Linux `d502e00d…`, macOS `2a7ce8f3…`); re-run `network-listings-release-sql.mjs` if you retag.* |
